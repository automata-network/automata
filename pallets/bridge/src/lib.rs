...
pub mod pallet {
	...
	pub use frame_support::{
		pallet_prelude::*, weights::GetDispatchInfo, Parameter
	};
	...
	impl<T: Config> Pallet<T> {
		...
		/// # </weight>
		#[pallet::weight(195_000_000)]
		pub fn whitelist_chain(origin: OriginFor<T>, id: BridgeChainId) -> DispatchResultWithPostInfo {
			T::AdminOrigin::ensure_origin(origin)?;
			Self::whitelist(id)
		}
	...
	impl<T: Config> Pallet<T> {
		...
		/// Cancels a proposal.
		fn cancel_execution(src_id: BridgeChainId, nonce: DepositNonce) -> DispatchResultWithPostInfo {
